...
| ~~C-8~~ | ✅ **shipped 2026-09-05.** Per-company board crawler — the missing primitive. `ingest-jobs` is the only thing that inserts board jobs and it walks FUNDS only; the per-ATS adapters are imported by `enrich-descriptions` alone, which fills descriptions and closes vanished rows but never discovers a posting. So a resolved `(ats_type, ats_token)` currently buys description coverage and liveness — NOT new jobs. Until this exists, C-1, C-2 and C-3 are all latent. Reuses `upsert_board_jobs` (write-once) and the `EXPIRY_GRACE_MS` vanished-sweep pattern, scoped per company. **Write-filtered to early career** per the pivot above. Prioritises the 679 companies already known to post early-career roles. **A live dry run against Stripe, Anthropic, Contra and Metronome crawled 1,214 jobs and kept 15 (1%)** — the filter is doing exactly its job, sparing 1,199 carousel generations. It also confirms the pivot's premise the hard way: Anthropic returned 594 jobs and **zero** early-career, Contra 5 and zero. **Startups barely post early-career roles, so C-8 alone will not fix the shortage — C-9 is required, not optional** | M | C-2 |
| ~~C-9~~ | ✅ **built 2026-09-05, insert not yet run.** External early-career seed — the highest-value item. VC boards produce startups, and startups barely hire interns; the pivot needs a company set that does. Simplify's two lists are literally that set — ~5,076 companies — and every record carries a raw ATS `url`, so `classifyApplyURL` yields `(ats_type, ats_token)` directly and dedupe lands on `companies_ats_unique` rather than weak name matching (their records have no domain field). Names + tokens only; postings still come from each company's own public ATS API. Licence: none on their repos, hence seed-only. `scripts/seed-early-career-companies.ts`, **dry run by default** — every seeded company gets crawled and every early-career job found costs a carousel plus an embedding, so the insert is an explicit spend decision.
**Measured 2026-09-05:** 36,281 records → 2,102 distinct boards → **1,773 NEW companies** (327 overlap with the 1,548 already held). That would take the address book from 1,548 to ~3,321 — more than double.
**And it sizes C-3 exactly:** of those 36,281 early-career records, **17,356 are Workday**, 3,592 Oracle/Taleo, 1,561 iCIMS, 345 Workable, 5,656 bespoke. **~80% of this market is on an ATS with no adapter.** Workday is not one option among several, it is the market.
Two bugs found by dry-running: Greenhouse's `boards.greenhouse.io/embed/job_app?token=…` widget URL was read as a board token named `embed`, collapsing 14 employers (Cerebras, Coinbase, Databricks, Dropbox…) onto a board that 404s — fixed in `classifyApplyURL`, which every ingest path uses. And boards claimed by 3+ distinct employers are now dropped as aggregators, while 2 spellings ("Match Group"/"Tinder") survive as the variants they are. Known residual: a single-name aggregator (`greenhouse/internshiplist2000`, attributed to Geotab) is not detectable by that rule — 1 row in 1,773 | M | C-8 |
| ~~C-3~~ | ✅ **shipped 2026-09-05, seed insert not yet run.** Workday adapter. Note after C-0: only **55 companies / 2,091 jobs** in the CURRENT corpus are on Workday, because that corpus is VC/startup-skewed. So C-3's value is **prospective** — it unlocks the enterprise-heavy intern/new-grad market scout22 is not in yet — not a repair of existing data. **Verified live, not assumed**: Boeing's `wday/cxs` endpoint answers unauthenticated with 435 intern postings, and Workday is the only supported ATS that SEARCHES SERVER-SIDE — so the early-career filter moves into the request instead of downloading a 3,000-job board to keep 40.
Three things it does not share with the other five: the address is a **triple** (`ats_token` = tenant, new `ats_host` = datacenter, new `ats_site` = career site — neither derivable, which is exactly why probing cannot reach Workday and the seed lists can); **`limit` caps at 20** and silently returns an empty page with a null total above that, so a naive port of the SmartRecruiters paging loop fetches nothing; and the list response is **sparse**, so `includeDetails: false` gives the crawler a cheap wide pass and `enrich-descriptions` fills JD bodies afterwards.
Live test: Boeing 316 fetched → 30 early-career, NVIDIA 369 → 82, Northrop 511 → 26, at 13-19s per company. **138 early-career roles from three companies, against 180 in the entire current corpus.**
Seed is ready: **1,451 Workday tenants** extracted with host+site. Insert pending — at ~15s per company this is ~40 hours of crawling, a materially bigger commitment than the first seed. Known pattern: `POST {tenant}.{host}/wday/cxs/{tenant}/{site}/jobs` with facet/pagination body. Multi-tenant, so the "token" is a (tenant, site) pair — `companies.ats_token` is a single column and will need widening | M–L | C-0 |
...
| C-10 | **Self-healing dead ATS tokens.** A company whose board is deleted keeps its dead token FOREVER and is never recovered. Observed 2026-09-06: Lumos (a16z), Loop (index-ventures), Harmonic.ai (craft-ventures), Inspiren (insight-partners), Black Forest Labs (general-catalyst) all 404 on both `/boards/{token}` and `/jobs`. **These were NOT probe errors** — `ats_probe_attempts = 0` on all five; the tokens were correct when `ingest-jobs` recorded them from a VC board, and the boards have since been deleted (ATS migration, acquisition, shutdown).
Today nothing handles this: `crawl-companies` logs the 404, stamps `last_crawled_at` and retries every 24h **indefinitely**, while `probe-ats-tokens` selects `ats_type is null` and so **can never revisit a company that has a dead-but-present token**. A company that moved Greenhouse → Ashby is permanently stuck.
Fix: count consecutive crawl failures on the company; after ~3, clear the coordinates so the prober picks it up again and may rediscover the new board. Dead → cleared → re-probed → recovered.
⚠ **The trap: clearing a WORKDAY token is destructive.** Its address includes a `site` that is not derivable from anything (guessing `RBCCareers` for RBC returns 404) — only a posting URL carries it, so probing cannot regenerate it. The rule must be ATS-aware: clear only what probing could plausibly rediscover (greenhouse/lever/ashby/smartrecruiters/recruitee), and for Workday either leave it or preserve `ats_host`/`ats_site` while clearing the type. Deferred deliberately rather than built during the 1,450-tenant Workday crawl, because it writes the same rows that crawl is touching | S–M | — |
| C-11 | **Optional: open the direct crawl beyond early career.** `crawl-companies` write-filters to intern/entry via `keepForEarlyCareerFeed`; the VC-board path (`ingest-jobs`) stays unfiltered, so startups still contribute every level and directly-crawled enterprises contribute only their early-career slice. That split is deliberate, matches the 2026-09-05 pivot, and is ONE LINE to reverse.
**Discarded jobs are not stored anywhere** — the filter runs before the upsert, so a mid/senior posting from a direct crawl is fetched into memory and dropped. Nothing to backfill from; opening up means re-crawling, which the 24h cadence does anyway.
**Measured 2026-09-06 across 47 runs / 1,105 companies: 38,394 jobs seen, 1,729 kept (4.5%), 36,665 discarded**, averaging ~35 postings per company board. So removing the filter is roughly a **22x** increase in direct-crawl volume — at 4,771 crawlable companies that is on the order of **165k jobs against a current corpus of 38.5k**.
The code change is trivial; the cost is not, and it is not the LLM bill. **Every active job needs a generated carousel** (99.8% carry one), so the binding constraint is `pitch-generate-carousel` throughput — see F9, where a backlog forced the schedule to `*/2` and left production drifting from the repo for two weeks. Workday would also need its `searchText` terms dropped, which removes the server-side filter that keeps enterprise boards affordable in the first place.
If reopening: stage it (one ATS or a company subset first), watch the carousel queue, and raise the cron **via a migration**, never `cron.alter_job` | S code / L consequences | carousel throughput |
