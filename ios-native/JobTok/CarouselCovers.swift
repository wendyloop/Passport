...
// MARK: - Giant type (brutalist poster)

struct GiantTypeCoverView: View {
    let slide: CoverSlide
    let style: CarouselStyle
    let job: JobPostingRecord
    ...
    private var words: [String] {
        (slide.role ?? job.title).uppercased().split(separator: " ").map(String.init)
    }

    private var metaLine: String {
        CoverFacts.build(slide: slide, job: job).prefix(3)
    ...
    /// Word with its tail dipped in accent — split point seeded per word so
    /// the same job always paints the same letters.
    private func dippedWord(_ word: String, index: Int) -> some View {
        let dip = min(max(1, 2 + Int(CarouselStyle.hash32(job.id + word) % 3)), max(word.count - 1, 1))
        let split = word.index(word.endIndex, offsetBy: -dip, limitedBy: word.startIndex) ?? word.startIndex
        let head = String(word[word.startIndex..<split])
        let tail = String(word[split...])
        return (Text(head).foregroundColor(theme.textPrimary) + Text(tail).foregroundColor(theme.accent))
